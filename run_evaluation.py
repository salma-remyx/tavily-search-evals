...
from utils.span_grounding_check import GROUNDING_CHECK_DEFAULT_CONFIG
...
def load_data(
    ...
    if evaluation_type == EvaluationType.DOCUMENT_RELEVANCE:
        return load_document_relevance_eval_data(
            dataset_path, start_index, end_index, random_sample
        )
    elif evaluation_type == EvaluationType.SIMPLEQA:
        return load_csv_data(dataset_path, start_index, end_index, random_sample)


async def get_search_handlers(
    search_provider_params: Dict[str, Dict[str, Any]], token_model: str = "gpt-4.1"
):
...
async def evaluate_provider_simple_qa(
    ...
    batch_size: int = 3,
    grounding_check_config: Optional[Dict[str, Any]] = None,
):
    """Evaluate a single search provider on the dataset."""
    grounding_check_config = grounding_check_config or GROUNDING_CHECK_DEFAULT_CONFIG
    ...
    async def process_example(example):
        ...
        try:
            ...
            answer = post_processor.extract_answer(
                query=query, is_llm_response=is_llm_response, search_result=search_ans
            )
            ...
            grounding_result = post_processor.check_answer_grounding(
                ...
                model=grounding_check_config.get("model"),
                temperature=grounding_check_config.get("temperature"),
            )
            # Evaluate the answer
            evaluation_result = await evaluator.evaluate(
                {"question": query}, {"answer": answer}, {"answer": reference_answer}
            )
            ...
            logger.info(
                f"[{provider_name}] Q{index}: Grade - {grade}, Query: '{query}'"
            )
            ...
            logger.error(
                f"[{provider_name}] Error evaluating example {index}: {str(e)}"
            )
            results.append(
                {
                    "index": index,
...
async def evaluate_provider_document_relevance(
    ...
    async def process_example(example):
        ...
        try:
            ...
            logger.error(
                f"[{provider_name}] Error evaluating example {example['index']}: {str(e)}"
            )
            error_result = {
                "index": example["index"],
                "question": query,
...
async def run_evaluation(
    ...
    Args:
        ...
        grounding_check_config: Grounding-judge settings (model/temperature) from
            configs/config.json's "grounding_check" block; falls back to
            GROUNDING_CHECK_DEFAULT_CONFIG when omitted
    """
    grounding_check_config = grounding_check_config or GROUNDING_CHECK_DEFAULT_CONFIG
    try:
        if "exa" in search_provider_params:
            if "contents" not in search_provider_params[
                "exa"
            ] or search_provider_params["exa"]["contents"] != {"highlights": True}:
                raise ValueError(
                    "'contents' field with 'highlights' is required for Exa. Please add it to the configuration."
                )
            ...
            for handler, provider_name in zip(search_handlers, provider_names):
                if evaluation_type == EvaluationType.SIMPLEQA:
                    task = evaluate_provider_simple_qa(
                        ...
                        post_processor,
                        evaluator_model,
                        grounding_check_config=grounding_check_config,
                    )
        ...
        if evaluation_type == EvaluationType.DOCUMENT_RELEVANCE:
            ...
            logger.info(
                f"Waiting for {QUOTIENT_AI_WAIT_TIME} seconds for logs to be processed in Quotient AI"
            )
            ...
            for provider_name, result in provider_results.items():
                ...
                if quotient_client:
                    ...
                    result["relevant_docs_percentage"] = (
                        (n_relevant_docs / n_docs * 100) if n_docs > 0 else 0
                    )
                    result["app_name"] = app_name

                    logger.info(
                        f"[{provider_name}] Relevance stats: {result['relevant_docs_percentage']:.1f}% ({result['relevant_docs']}/{result['total_docs']})"
                    )
        ...
        for provider_name, result in provider_results.items():
            if evaluation_type == EvaluationType.SIMPLEQA:
                print(
                    f"{provider_name}: {result['accuracy']:.2%} ({result['correct_count']}/{result['total_count']})"
                )
            elif evaluation_type == EvaluationType.DOCUMENT_RELEVANCE:
                print(
                    f"{provider_name}: {result['relevant_docs_percentage']:.1f}% ({result['relevant_docs']}/{result['total_docs']})"
                )
        print("=============================\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run benchmark evaluation using specified evaluation type"
    )
    parser.add_argument(
        "--evaluation_type",
        ...
        help="Type of evaluation to run (simpleqa or document_relevance)",
    )
    parser.add_argument(
        "--config",
        default="configs/config.json",
        type=str,
        help="Path to JSON config file with provider parameters",
    )
    parser.add_argument(
        "--start_index",
        type=int,
        default=0,
        help="Starting index for examples (inclusive)",
    )
    parser.add_argument(
        "--end_index",
        type=int,
        default=None,
        help="Ending index for examples (exclusive)",
    )
    parser.add_argument(
        "--random_sample",
        type=int,
        default=None,
        help="Number of random samples to select (overrides start/end index)",
    )
    parser.add_argument(
        "--post_process_model",
        default="gpt-4.1",
        help="Model for post-processing for SimpleQA",
    )
    parser.add_argument(
        "--token_model",
        default="gpt-4.1",
        help="Model for token consumption calculation",
    )
    parser.add_argument(
        "--evaluator_model",
        default="gpt-4.1",
        help="Model for correctness evaluation for SimpleQA",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rerun",
        action="store_true",
        help="Rerun evaluation on existing results directory, output_dir must exist",
    )
    ...
    # Grounding-check settings live in config alongside the providers; pull them
    # out so they aren't treated as a search provider, falling back to the
    # module-level default when the config file omits them or fails to load.
    grounding_check_config = search_provider_params.pop(
        "grounding_check", GROUNDING_CHECK_DEFAULT_CONFIG
    )

    evaluation_type = EvaluationType(args.evaluation_type)
    output_dir = get_output_dir(
        evaluation_type=evaluation_type, output_dir=args.output_dir, rerun=args.rerun
    )
